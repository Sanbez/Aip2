...
import { X, Clock, Users, Calendar, Tag, Building, Share2, Heart, ChevronLeft, ChevronRight, Award, Flame } from "lucide-react"
...
export function EventModal({ event, onClose }: EventModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-5xl max-h-[90vh] overflow-auto bg-card border border-border rounded-2xl shadow-2xl animate-in zoom-in-95 fade-in">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="lg:w-1/2 p-4 lg:p-6 lg:border-l border-border space-y-5">
            ...
            <Button size="lg" className="w-full text-lg h-14">
              {event.isFree ? "Зарегистрироваться" : `Купить билет за ${event.price.toLocaleString("ru-RU")} ₽`}
            </Button>
          </div>
        </div>
      </div>
